...
In this section I showcase the most used, \emph{trendy} JavaScript parser technologies and justify why I have chosen the Shapesecurity Shift family as the parser and additional toolset for my approach.
...
\subsubsection{Speed} Although speed is not the most important property of a system whose aim is to make sure no errors are present, it is wanted for a nearly real-time response system. \Cref{table:speed-comparison-of-parsers} shows the time difference parsers process different source codes. The benchmark~\footnote{\url{http://esprima.org/test/compare.html}} was run on a personal computer not set up for proper benchmarking. Its sole purpose is to get a rough comparison between the different technologies available.
