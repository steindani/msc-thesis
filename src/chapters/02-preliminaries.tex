...
In order to make sure a system behaves correctly according to the specification, a broad range of \emph{formal methods} are present. Besides \emph{model checkers}, \emph{run-time monitoring}, the most popular formal methods are \emph{type systems}.~\cite{pierce2002types} \textquote[\cite{pierce2002types}]{A type system is a syntactic method for automatically checking the absence of certain erroneous behaviors by classifying program phrases according to the kinds of values they compute.}

Type inference refers to the deduction of the data types of expressions, statements in the source code, usually executed during compilation and static analysis. By utilizing type inference, deducing types as \emph{interfaces}, and checking contracts between various parts of the software can yield a more consistent and bug-free code.

Type systems also help developers write the source code with context-aware assistance. With implicitly typed languages the ability to infer data types can make prototyping and developing programs in a more agile fashion, compared to explicitly typed languages. Omiting the type annotations eliminates the need to propagate changes in the source code in case of a refactor, while still executing interface checks prevent type errors at runtime.
