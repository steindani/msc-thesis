...
A simple addition (a \code{BinaryExpression} with a \code{+} sign as a discriminating \code{BinaryOperator}) can have various results~\cite{js-arithmetic-operators}:
\begin{itemize}[topsep=0pt]
	\item \code{Number  + Number  $\longrightarrow$ Number} \hfill\emph{(addition)}
	\item \code{Boolean + Number  $\longrightarrow$ Number} \hfill\emph{(addition)}
	\item \code{Boolean + Boolean $\longrightarrow$ Number} \hfill\emph{(addition)}
	\item \code{Number  + String  $\longrightarrow$ String} \hfill\emph{(concatenation)}
	\item \code{String  + Boolean $\longrightarrow$ String} \hfill\emph{(concatenation)}
	\item \code{String  + String  $\longrightarrow$ String} \hfill\emph{(concatenation)}
\end{itemize}

Logical operators also behave differently compared to e.g., Java. The logical and operator (\code{\&\&}) returns the left expression if it is so-called \emph{falsy} (values that can be converted to \code{false}, such as \code{null}, 0, undefined). If the left side expression is \emph{truthy}, it returns the right side value. Thus the possible return \emph{type} of the expression can be both types and not necessarily \code{Boolean}.
