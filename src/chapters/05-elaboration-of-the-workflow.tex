...
These \code{Reference}s are only present for static scope variable accesses. New transformation rules have to be created covering the cases when an \code{Object} is dynamically introduced to a scope --- e.g., passed as a parameter for a function. This is also subject to future work.
...
In order to achieve better language coverage it is subject to future work to develop a proper representation for compound and complex data structures, handle built-in functions and methods of data types. Challenges also include handling the \code{this} references in functions and class methods; anonym functions and method calls like \code{forEach}, \code{map}, or \code{filter}.

With the right constraints it is possible to approximately infer the type of a given expression and help developers with context-aware information and warning for type-related problems.
